tonyPAMGuide_Meta <- function(fullfile,...,atype='PSD',plottype='Both',envi='Air',calib=0,ctype = 'TS',Si=-159,Mh=-36,G=0,vADC=1.414,r=50,N=Fs,winname='Hann',lcut=Fs/N,hcut=Fs/2,timestring="",outdir=dirname(fullfile),outwrite=0,disppar=1,welch="",chunksize="",linlog = "Log", StartTime = NA, CallOnset = NA, seconds = NA){ # StartTime, CallOnset, and seconds added as arguments; if they are not specified, then the function should run equivalent to PAMGuide_Meta
  ...
  ### Tony: this bit was changed to potentially read a windows of "seconds" from the file starting that window before CallOnset
...
##### NEW SECTION FROM VIEWER.R FUNCTION TO CALCULATE LEVELS
          RMSlev <- 10*log10(mean(10^(a/10)))
            ...
            # the two lines below are new as of 2021-07-12 and return
